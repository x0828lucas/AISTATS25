...
# Data Preparation 
## ----------------
data = np.loadtxt('diabetic_multiclass.csv', delimiter=',')
n = data.shape[0]
n_train = int(0.5*n)
n_valid = int(0.2*n)
n_test = int(0.25*n)
...
# Hyper-Parameters
## ----------------
sample_ratios = [0.0006, 0.001, 0.005, 0.01, 0.02]
size_subgroup = 10
num_class = 3
k = 5
iterations = 10
...
for sample_ratio in sample_ratios:
    ...
    for iteration in range(iterations):
        ...
        while len(np.unique(y_train[idx_subtrain]))<num_class:
            idx_subtrain = np.random.choice(n_train, size=size_compressed, replace=True)
        ...
        for i in range(num_subgroup):
            ...
            if i==0:
                ...
                y_pred_test = y_pred_test + y_pred_prob #np.concatenate([y_pred_group, y_pred_prob], axis=1)
